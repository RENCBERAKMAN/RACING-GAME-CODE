...
void AMyProjectSportsCar::Tick(float DeltaTime)
{
	...
	{
		return; // Oyuncu kontrolünde ise AI çalışmaz
	}

	if (!SplineComp) return;

	float Speed = GetVelocity().Size();
	const float TargetSpeed = 270.f; // Sabit hedef hız
	...
	if (Speed < TargetSpeed)
	{
		...
		FVector EkstraForce = GetActorForwardVector() * 200000.f;
		GetMesh()->AddForce(EkstraForce);
	}
	else
	{
		GetChaosVehicleMovement()->SetThrottleInput(1.0f);
	...
	if (Speed > TargetSpeed + 50.f)
	{
		GetChaosVehicleMovement()->SetBrakeInput(0.2f);
	}
	else
	{
