...
AMyProjectPawn::AMyProjectPawn()
{
	...
	LeftHeadlight->SetRelativeLocation(FVector(160.f, -40.f, 75.f)); // aracına göre ayarla
	LeftHeadlight->SetIntensity(40000.f); // parlaklık
	LeftHeadlight->SetLightColor(FLinearColor::White);
	LeftHeadlight->SetOuterConeAngle(40.f);
	LeftHeadlight->SetInnerConeAngle(20.f);
	LeftHeadlight->SetAttenuationRadius(8000.f); // farın menzili
	LeftHeadlight->SetCastShadows(false);


	// Sağ far
	RightHeadlight = CreateDefaultSubobject<USpotLightComponent>(TEXT("RightHeadlight"));
	RightHeadlight->SetupAttachment(GetMesh());
	RightHeadlight->SetRelativeLocation(FVector(160.f, 40.f, 75.f));
	RightHeadlight->SetIntensity(40000.f);
	RightHeadlight->SetLightColor(FLinearColor::White);
	RightHeadlight->SetOuterConeAngle(40.f);
	RightHeadlight->SetInnerConeAngle(20.f);
	RightHeadlight->SetAttenuationRadius(8000.f);
	RightHeadlight->SetCastShadows(false);
...
void AMyProjectPawn::SetThrottleInput(float Value)
{
	if (ChaosVehicleMovement)
		ChaosVehicleMovement->SetThrottleInput(Value);
}

void AMyProjectPawn::SetBrakeInput(float Value)
{
	if (ChaosVehicleMovement)
		ChaosVehicleMovement->SetBrakeInput(Value);
}

void AMyProjectPawn::SetSteeringInput(float Value)
{
	if (ChaosVehicleMovement)
